...
if (true) {
  $stmt = $conn->prepare("SELECT DISTINCT MIJN_LENINGEN.in_bezit, MIJN_LENINGEN.product_id_fk ,MIJN_LENINGEN.isTerugGebracht, MIJN_LENINGEN.isVerlenged, MIJN_LENINGEN.Uitleendatum, MIJN_LENINGEN.terugbrengDatum, GROEP.naam AS groep_naam, PRODUCT.product_id, lening_id
  FROM MIJN_LENINGEN
      INNER JOIN PRODUCT ON MIJN_LENINGEN.product_id_fk = PRODUCT.product_id
      INNER JOIN GROEP ON PRODUCT.groep_id = GROEP.groep_id
  WHERE user_id_fk = $userID && MIJN_LENINGEN.isTerugGebracht = false
